...
st.markdown(
    ...
    <style>
        ...
        * { font-family: 'DM Sans', sans-serif; }
        ...
        .right-panel-spacer { height: 2.2rem; }

        .right-panel {
            border: 1.5px solid var(--antihub-pink);
        ...
        .stat-box { background: var(--antihub-blush); color: var(--antihub-ink); border: 1px solid rgba(139, 50, 91, 0.10); border-radius: 1rem; padding: 0.9rem 0.5rem 1rem 0.5rem; text-align: center; margin-bottom: 0; }
        ...
        .current-file { font-size: 0.78rem; margin: 0 0 0.8rem 0; color: var(--antihub-ink); }
...
with right:
    ...
    st.markdown('<div class="helper-text">Enter the receipt amount. Buttons copy it and fire Keyboard Maestro.</div>', unsafe_allow_html=True)

    amount_col, copy_col, clear_col, undo_col = st.columns([1.45, 0.72, 0.72, 0.9])
    with amount_col:
        amount_input = st.text_input("Amount", key="qbo_search_amount", placeholder="117.98", label_visibility="collapsed")
    ...
    with clear_col:
        if st.button("Clear", use_container_width=True):
            ...
            st.rerun()
    with undo_col:
        if st.button("Undo", use_container_width=True, disabled="last_move" not in st.session_state):
    ...
    show_status()
    st.markdown('<div class="small-note">Chrome should already have the correct client QBO file available. Keyboard Maestro handles the clicky nonsense.</div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
